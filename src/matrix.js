...
$doodle.Matrix.translate = function (m, dx, dy) {
	return $doodle.Matrix.multiply(m, [1,0,0,1,dx,dy]);
};

$doodle.Matrix.rotate = function (m, angle/*radians*/) {
	var sin = Math.sin(angle);
	var cos = Math.cos(angle);
	return $doodle.Matrix.multiply(m, [cos,sin,-sin,cos,0,0]);
};

$doodle.Matrix.scale = function (m, sx, sy) {
	return $doodle.Matrix.multiply(m, [sx,0,0,sy,0,0]);
};

//returns opposite transform of original matrix. to undo a transform
$doodle.Matrix.invert = function (m) {
	var _m = new Array(6);		
...
$doodle.Matrix.transformPoint = function (m, x, y) {
	return {
		x: m[0] * x + m[1] * y + m[4],
		y: m[2] * x + m[3] * y + m[5]
	};
...
$doodle.Matrix.deltaTransformPoint = function (m, x, y) {
...
$doodle.Matrix.rotateAroundInternalPoint = function (m, x, y, angle) {
	var _m = $doodle.Matrix.translate(m, x, y);
	_m = $doodle.Matrix.rotate(_m, angle * Math.PI/180);
	_m = $doodle.Matrix.translate(_m, -x, -y);
	return _m;
};

$doodle.Matrix.rotateAroundExternalPoint = function (m, x, y, angle) {
	var _m = m.concat();
